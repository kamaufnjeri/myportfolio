  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status == 201) {
        toast.success(`${response.data.message}: Hi ${response.data.user.email}!`)
      ...
      <form onSubmit={(e) => handleSubmit(e)}>
        <div className={styles.inputbox}>
            <input 
                ...
                value={ data.email }
            />
            <span className={styles.label} onClick={() => focusInput(emailRef)}>Enter Email</span>
        </div>
        <div className={styles.inputbox}>
            <input 
                ref={passwordRef}
                ...
                value={ data.password }
            />
             <span className={styles.label} onClick={() => focusInput(passwordRef)}>Enter Password</span>
        </div>
